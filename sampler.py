...
def clique_negative_sampling(
    ...
    while neg in hyperedges:
        ...
        check_probable_neighbors = torch.stack(probable_neighbors)
        neighbor_node = np.random.choice(check_probable_neighbors.cpu(), size=1)[0]
...
def corrupt_negative_sampling(
    ...
    while neg in hyperedges:
        ...
        check_edge = torch.stack(edge)
        nodes_to_keep = list(set(check_edge.cpu()) - set(np.array(check_edge.cpu())[node_to_remove]))
        probable_neighbors = node_set - list_hyperedges[edgeidx]
        probable_neighbors = list(probable_neighbors)
        check_probable_neighbors = torch.stack(probable_neighbors)
        neighbor_node = np.random.choice(check_probable_neighbors.cpu(), size=corrupt_num)        
...
class CNSBatch(object):
    def __init__(self, batch_size, hedges, device):
        self.hedges = set([frozenset(x.cpu()) for x in hedges])
        self.bs = batch_size
        self.device = device
    def __call__(self):
